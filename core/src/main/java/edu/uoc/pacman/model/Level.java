...
public class Level {
    ...
    public Iterator<MapItem> getMapItemListIterator() {
        return getMapItemList().iterator();
    }
    ...
    public MapItem getMapItem(int x, int y) {
        Iterator itr = getMapItemListIterator();
        while(itr.hasNext()){
            MapItem mapItem = (MapItem) itr.next();
            if(mapItem.getPosition().getX() == x && mapItem.getPosition().getY() == y){
                return mapItem;
            }
        }
        return null;
    ...
    public MapItem getMapItem(Position position) throws NullPointerException {
        if(position == null){
            ...
            while(itr.hasNext()){
                MapItem mapItem = (MapItem) itr.next();
                if(mapItem.getPosition().equals(position)){
                    return mapItem;
                }
            }
        }
    ...
    public Blinky getBlinky() {
        Iterator itr = getGhostList().iterator();
        while(itr.hasNext()){
    ...
    public boolean hasWon() {
        ...
        while(itr.hasNext()){
            MapItem mapItem = (MapItem) itr.next();
            if(mapItem.getClass().equals(Dot.class) || mapItem.getClass().equals(Energizer.class)){
                return false;
            }
        }
        return true;
